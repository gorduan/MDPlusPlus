 ...
 *
 * This extension is contributed by the Mermaid plugin.
 */

import { Node, mergeAttributes, InputRule } from '@tiptap/core';
import { ReactNodeViewRenderer, type ReactNodeViewRendererOptions } from '@tiptap/react';

// Use a simpler type that is compatible with ReactNodeViewRenderer
type NodeViewComponent = Parameters<typeof ReactNodeViewRenderer>[0];

export interface MermaidBlockOptions {
  HTMLAttributes: Record<string, unknown>;
  /** Custom NodeView component - injected at registration time */
  nodeViewComponent?: NodeViewComponent;
}
...
export const MermaidBlock = Node.create<MermaidBlockOptions>({
  ...
  addNodeView() {
    if (this.options.nodeViewComponent) {
      return ReactNodeViewRenderer(this.options.nodeViewComponent);
    }
    // Fallback: no visual editor, just render the code block
    return null;
  },
